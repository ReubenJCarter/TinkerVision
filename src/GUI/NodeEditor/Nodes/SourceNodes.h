...
class GraphInputNode: public BaseNode
{
	...
	public:
		GraphInputNode()
        {
            ...
            comboEdit->setMaximumSize( QSize(150, 50) );

            comboEdit->addItem("intData");
            comboEdit->addItem("floatData");
            comboEdit->addItem("boolData");
            comboEdit->addItem("stringData");
            comboEdit->addItem("imageData"); 
            comboEdit->addItem("imageGPUData");
            comboEdit->addItem("imageTypeData"); 
            ...
            connect(comboEdit, QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int inx)
            {
                ...
                std::cout << "changed type:" << dataType << "\n";
                _outputPorts[0].type = BaseNodeData("intData", "IntData").type();
                data = std::make_shared<BaseNodeData>(dataType, name);
                SetValidationState(QtNodes::NodeValidationState::Valid, ""); 
                Q_EMIT dataUpdated(0);
            });
            
            data = std::make_shared<BaseNodeData>("intData", "IntData");
        ...
        QJsonObject save() const 
        {
            ...
            if(data) modelJson["graphInputInx"] = QString::number(graphInputInx);
            return modelJson;
        }
        ...
        { 
            std::cout << "output: " << data->type().id.toStdString() << " " << data->type().name.toStdString() << "\n";
